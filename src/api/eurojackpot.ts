...
export async function saveDrawsToBackend(uploadToServer: boolean = false, drawsToSave?: PersistableEuroJackpotDraw[]): Promise<any> {
  ...
  if (drawsToSave && normalizedDraws) {
    ...
    if (inputCount >= minCountForGuard && retentionRatio < minRetentionRatio) {
      const message = `Suspicious dedup drop detected for EuroJackpot: ${inputCount} -> ${outputCount}. Save aborted to protect server data.`
...
export async function fetchSavedDrawsFromServer(): Promise<{ draws: EuroJackpotDraw[]; lastFetchedAt: string | null }> {
  try {
    const response = await fetch(`${READ_DRAWS_ENDPOINT}?gameType=eurojackpot&t=${Date.now()}`, {
      headers: {
...
function sortAndDeduplicate(draws: EuroJackpotDraw[]): EuroJackpotDraw[] {
  const getCompletenessScore = (draw: EuroJackpotDraw): number => {
    ...
    return (mainCount * 100) + (euroCount * 10) + hasPositiveSystemId
  }

  const uniqueByKey = new Map<string, EuroJackpotDraw>()
  for (const draw of draws) {
    const dateKey = toDateKey(draw.drawDate)
    const mainKey = Array.isArray(draw.numbers) ? draw.numbers.join(',') : ''
    const euroKey = Array.isArray(draw.euroNumbers) ? draw.euroNumbers.join(',') : ''
    const fallbackId = typeof draw.drawSystemId === 'number' ? draw.drawSystemId : 0
    const dedupeKey = dateKey
      ? `${dateKey}|${mainKey}|${euroKey}`
      : `id:${fallbackId}|${mainKey}|${euroKey}`
